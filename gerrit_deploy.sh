...
function gerrit_deploy() {
    if [ $(get_config GERRIT_DEPLOY_TYPE) -eq 1 ]; then
        # 测试环境部署
        echo ">>> 开始测试环境部署..."
        sudo docker run --name gerrit_dev --restart=always -itd -p $(get_config GERRIT_DEPLOY_PORT):8080 -p $(get_config GERRIT_SERVER_PORT):29418 gerritcodereview/gerrit:$(get_config GERRIT_DEPLOY_VERSION)
    elif [ $(get_config GERRIT_DEPLOY_TYPE) -eq 2 ]; then
        # 正式环境部署
        echo ">>> 开始正式环境部署..."
    else
        echo "不支持的部署类型"
    ...
    echo ">>> 部署完毕，您可以通过以下地址访问您的gerrit:"
    echo
     echo "http://127.0.0.1:$(get_config GERRIT_DEPLOY_PORT)"
}
