...
{
    # 删除单独的 manifests 仓库
    echo "ssh -p $GERRIT_SERVER_PORT $GERRIT_SERVER_USERNAME@$GERRIT_SERVER_IP delete-project delete --yes-really-delete platform/manifests"
    ssh -p $GERRIT_SERVER_PORT $GERRIT_SERVER_USERNAME@$GERRIT_SERVER_IP delete-project delete --yes-really-delete platform/manifests

    # 删除源码
    for i in `cat $OUTPUT_PROJECT_LIST_FILE_NAME`;
    do
        echo $i
        echo "ssh -p $GERRIT_SERVER_PORT $GERRIT_SERVER_USERNAME@$GERRIT_SERVER_IP delete-project delete --yes-really-delete $i"
        ssh -p $GERRIT_SERVER_PORT $GERRIT_SERVER_USERNAME@$GERRIT_SERVER_IP delete-project delete --yes-really-delete $i
    done

    # 删除父仓库
    echo "ssh -p $GERRIT_SERVER_PORT $GERRIT_SERVER_USERNAME@$GERRIT_SERVER_IP delete-project delete --yes-really-delete AOSP"
    ssh -p $GERRIT_SERVER_PORT $GERRIT_SERVER_USERNAME@$GERRIT_SERVER_IP delete-project delete --yes-really-delete AOSP
}
